...
import { runMigrate } from '../../../electron/main/migrate-axecoder'
import { setAxecoderDirForTests, getAxecoderDir } from '../../../electron/main/axecoder-dir'
...
describe('migrate-axecoder', () => {
  it('从 legacy 字段迁移一条 openai 模型', async () => {
    ...
    const secretsRaw = await fs.readFile(path.join(getAxecoderDir(), 'secrets.json'), 'utf-8')
