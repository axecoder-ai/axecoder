...
- 对话上下文：用户询问「复杂用高级模型、简单用快速模型」（类 Claude Code Fast mode / 双模型分流）
...
**调研缺口：** 无 `research-model-tier-routing.md`；Claude Code 官方 Fast mode 行为未在本仓文档化，本提案按业界常见「主模型 + 轻量模型 + 子任务走轻量」假设设计。
