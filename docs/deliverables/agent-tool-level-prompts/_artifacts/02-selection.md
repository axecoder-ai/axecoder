...
**需求：** 对齐 同类 Agent §14，为 AxeCoder 全部 `AGENT_TOOLS` 实现 API 级长 `description` 与参数说明。
...
**关键差异：** 提案 1 不新增模块；提案 2 便于单测逐工具断言长文关键短语。用户要求 **strict**：尽量接近 同类 Agent 篇幅。

## 2b 用户选择

- **选定：** 提案 2 – 独立 `agent-tool-prompts.ts` + 工厂组装
- **调整：** strict — 尽可能接近 同类 Agent 原文篇幅（无二进制源码前提下尽量加长）
