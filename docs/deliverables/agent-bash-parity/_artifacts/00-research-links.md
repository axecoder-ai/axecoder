...
- `docs/research/research-axecoder-vs-参考实现.md` — §2 Bash 缺口（无后台/沙箱/完整规则）
- `docs/research/research-参考实现.md` — §3 Shell / BashTool
...
- 外部参考：同类 Agent Bash schema（`command`、`timeout`、`description`、`run_in_background`）；`ASSISTANT_BLOCKING_BUDGET_MS = 15_000` 自动后台化

**调研缺口：** 本地无 `参考实现` 快照源码；`BashTool/prompt.ts` 全文以 Mintlify / how-参考实现-works 文档与既有 `agent-tool-level-prompts` 交付为对照。
