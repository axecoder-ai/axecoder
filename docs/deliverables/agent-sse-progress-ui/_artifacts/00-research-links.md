...
- `docs/research/research-axecoder-vs-claude-code.md` §5 — 子代理 UI 进度对比
- Claude Code 文档（CLI）：spinner verbs、terminal progress bar、fullscreen TUI（外部参考，仓库无 claude-code 源码）

## 调研缺口

- 本仓库 `claude-code/` 无 CLI 渲染源码；终端工具行样式依据 Claude Code 公开文档与常见终端 UX（单行工具名 + 参数、活动行 shimmer、推理块弱化）做 IDE 等价实现。
