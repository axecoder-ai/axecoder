...
- `docs/research/research-axecoder-vs-参考实现.md` — §2 Agent 类型、§5 子代理 UI（部分条目已过时，见下）
...
**调研缺口：** 仓库内无 Cursor Composer 子代理运行时源码；按 Task 工具公开契约 + 现有 AxeCoder 实现 diff 推导缺口。同类 Agent 自定义 `agents/` 目录仅作参考，非 CC 必选。
