...
## 4. Claude Code 同源、部分会话可见的工具

Cursor Agent 与 Claude Code 工具层有重叠；下列工具**不一定每次会话都注入**，但在 playbook / 技能 / 部分构建中会出现。
...
**完整六列矩阵**（Cursor · DeepSeek-TUI · Claude Code · OpenCode · Reasonix · AxeCoder）见：
