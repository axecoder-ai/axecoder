...
import { ensureWritcraftDir, getWritcraftDir } from './writcraft-dir'
...
export const runMigrate = async (opts: MigrateOptions = {}) => {
  ...
  const modelsPath = path.join(getWritcraftDir(), 'models.json')
  ...
  if (opts.legacyChatPath) {
    const dest = path.join(getWritcraftDir(), 'chat-sessions.json')
    try {
